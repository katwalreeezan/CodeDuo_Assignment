...
const favoriteSpellsSlice = createSlice({
  ...
  reducers: {
    addToFavorites: (state, action: PayloadAction<Spell>) => {
      ...
      localStorage.setItem('favoriteSpells', JSON.stringify(state.favoriteSpells));
    },
    removeFromFavorites: (state, action: PayloadAction<Spell>) => {
      state.favoriteSpells = state.favoriteSpells.filter(spell => spell.index !== action.payload.index);
      localStorage.setItem('favoriteSpells', JSON.stringify(state.favoriteSpells));
    },
  },
});

export const { addToFavorites, removeFromFavorites } = favoriteSpellsSlice.actions;
